...
class HeatmapKeypointModel(nn.Module):
    def __init__(
        ...
        """
        super().__init__()
        self.backbone = self.create_resnet_backbone(freeze_initial_layers=True)
        ...
        # Calculate required deconvolution steps
        ...
        for _ in range(num_deconv_layers):
            ...
            self.deconv_layers.append(nn.BatchNorm2d(deconv_channels))

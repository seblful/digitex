"""Tests for the review seam — chiefly the numbering rule it enforces.

`numbering_fault` is what replaced the renumbering command: rather than
repairing a tree that went out of order, the review window refuses to write it
that way. It reads the disk but needs no display, so it is testable here.
"""

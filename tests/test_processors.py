...
class TestResizeImage:
    ...
    def test_resize_image_pads_to_max_dimensions(self) -> None:
        """Test that image smaller than max dimensions is padded to max."""
        img = Image.new("RGB", (100, 100), color="red")
        result = resize_image(img, 200, 200)
        assert result.size == (200, 200)

    def test_resize_image_preserves_aspect_ratio_width_limited(self) -> None:
        """Test that aspect ratio is preserved when width is the limiting factor."""
        img = Image.new("RGB", (400, 200), color="red")
        result = resize_image(img, 200, 200)
        assert result.size == (200, 100)

    def test_resize_image_preserves_aspect_ratio_height_limited(self) -> None:
        """Test that aspect ratio is preserved when height is the limiting factor."""
        img = Image.new("RGB", (200, 400), color="red")
        result = resize_image(img, 200, 200)
        assert result.size == (100, 200)

    def test_resize_image_square_to_portrait(self) -> None:
        """Test resizing a landscape image to portrait."""
        img = Image.new("RGB", (200, 100), color="red")
        result = resize_image(img, 100, 100)
        assert result.size == (100, 50)

    def test_resize_image_square_to_landscape(self) -> None:
        """Test resizing a portrait image to landscape."""

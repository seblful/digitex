...
Notes:
-----
* Primary keys use ``BIGINT GENERATED BY DEFAULT AS IDENTITY`` so that
  callers (e.g. ad-hoc data loaders) can insert explicit ids with
  ``OVERRIDING SYSTEM VALUE`` if needed.
* ``images.question_id``, ``question_topics.question_id``, and
